...
[![最后提交](https://img.shields.io/github/last-commit/agentscope-ai/Minions)](https://github.com/agentscope-ai/Minions)
...
[![GitHub Fork](https://img.shields.io/github/forks/agentscope-ai/Minions?style=flat&logo=github&color=purple&label=Fork)](https://github.com/agentscope-ai/Minions/network)
...
<a href="https://trendshift.io/repositories/25882" target="_blank"><img src="https://trendshift.io/api/badge/trendshift/repositories/25882/daily" alt="agentscope-ai%2FMinions | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>

[[文档](https://minions.agentscope.io/)] [[English](README.md)] [[日本語](README_ja.md)] [[Русский](README_ru.md)]

<p align="center">
  <img src="https://gw.alicdn.com/imgextra/i1/O1CN01sens5C1TuwioeGexL_!!6000000002443-55-tps-771-132.svg" alt="Minions Logo" width="120">
</p>
...
| **本地云端，自由运行**                    | Minions-Flash 模型（2B / 4B / 9B）面向 Agent 场景专项训练。内置 Minions Local 运行时，无需 API Key、不依赖云端。同时支持 Ollama、LM Studio 及 14+ 云端模型供应商。                  |
...
<details>
<summary><b>你可以用 Minions 做什么</b></summary>
...
- [2026-06-17] **v1.1.12 — 模型页面大改版 & 简洁模式** | 全新模型页面 Provider 聚合设计；新增简洁模式，扁平化导航。
  ...
  另有：宽屏模式、代码语法高亮、Langfuse agent loop 分组、Gemini 工具 schema 修复。[v1.1.12 发布说明 →](https://minions.agentscope.io/release-notes#v1.1.12)

- [2026-06-11] **AgentScope 体验平台上线** — 支持免费部署 Minions、插件分享与 Skill 市场。[立即体验 →](https://platform.agentscope.io/)

- [2026-06-10] **v1.1.11** — 免费模型 OAuth、插件市场、MCP 工具白名单。[v1.1.11 发布说明 →](https://minions.agentscope.io/release-notes#v1.1.11)

- [2026-06-01] **v1.1.10** — 子 Agent 派生、打开目录、腾讯元宝频道。[v1.1.10 发布说明 →](https://minions.agentscope.io/release-notes#v1.1.10)

[全部发布说明 →](https://minions.agentscope.io/release-notes)
...
然后在浏览器中打开控制台 **http://127.0.0.1:8088/** 配置模型。若要在钉钉、飞书、微信等 app 内对话，请参考 [频道配置](https://minions.agentscope.io/docs/channels) 文档。
...
无需手动配置 Python，一行命令自动完成安装。脚本会自动下载 uv（Python 包管理器）、创建虚拟环境、安装 Minions 及其依赖（含 Node.js 和前端资源）。注意：部分网络环境或企业权限管控下可能无法使用。
...
>    - **找到安装目录**：
>      - 检查 `uv` 是否可用：在 CMD 中输入 `uv --version`，如果显示版本号，则**只需配置 Minions 路径**；如果提示 `'uv' is not recognized as an internal or external command, operable program or batch file,`，则需同时配置两者。
>      - uv 路径（任选其一，取决于安装位置，若 `uv` 不可用则填）：通常在 `%USERPROFILE%\.local\bin`、`%USERPROFILE%\AppData\Local\uv` 或 Python 安装目录下的 `Scripts` 文件夹
>      - Minions 路径：通常在 `%USERPROFILE%\.minions\bin`。
>    - **手动添加到系统的 Path 环境变量**：
...
```bash
minions init --defaults   # 或：minions init（交互式）
minions app
```
...
# 卸载
minions uninstall          # 保留配置和数据
minions uninstall --purge  # 删除所有内容
```
...
镜像在 **Docker Hub**（`agentscope/minions`）。镜像 tag：`latest`（稳定版）；`pre`（PyPI 预发布版）。
...
国内用户也可选用阿里云容器镜像服务 (ACR)：`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/minions`（tag 相同）。

然后在浏览器中打开 **http://127.0.0.1:8088/** 进入控制台。配置、记忆与 Skills 保存在 `minions-data` 卷中；模型提供商设置与 API Key 保存在 `minions-secrets` 卷中；备份归档保存在 `minions-backups` 卷中。如需传入 API Key（如 `DASHSCOPE_API_KEY`），在 `docker run` 时添加 `-e VAR=value` 或 `--env-file .env`。

> **从容器内连接宿主机上的 Ollama 或其他模型服务**
>
> Docker 容器内的 `localhost` 指向容器自身，而非宿主机。如果 Ollama（或其他模型服务）运行在宿主机上，可通过以下方式让容器内的 Minions 访问：
>
...
>   agentscope/minions:latest
> ```
> 然后在 Minions **设置 → 模型** 中，将 Base URL 改为 `http://host.docker.internal:<端口>` — 例如 Ollama 填 `http://host.docker.internal:11434`，LM Studio 填 `http://host.docker.internal:1234/v1`。
>
...
若希望将 Minions 部署在阿里云上，可使用阿里云 ECS 一键部署：打开 [Minions 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) 按页面提示操作即可。详细步骤见 [阿里云开发者社区：3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。

---

### 方式五：AgentScope Platform 部署

[AgentScope Platform](https://platform.agentscope.io/) 提供一键云端 Minions 部署、插件分享与 Skill 市场。完全免费，7/24 在线。

---

### 方式六：使用魔搭创空间

使用 [魔搭创空间](https://modelscope.cn/studios/fork?target=AgentScope/Minions) 也可进行云端 Minions 部署。注意：请将创空间设为 **非公开**，否则他人可能操纵你的 Minions。
...
从 [官方下载页](https://minions.agentscope.io/downloads) 下载桌面应用（Tauri 版）：
- **Windows**：`Minions-Tauri-<version>-Windows-setup.exe`
- **macOS**：`Minions-Tauri-<version>-macOS.zip`（推荐 Apple Silicon）
...
当你从 Releases 下载 Minions macOS 应用时，macOS 可能显示：*"Apple 无法验证 'Minions' 不包含恶意软件"*。这是因为应用未经过公证。你仍然可以通过以下方式打开：

- **右键打开（推荐）**
  右键点击（或 Control + 点击）Minions 应用 → **「打开」** → 在对话框中再次点击 **「打开」**。这会告诉 Gatekeeper 你信任该应用；之后可以像往常一样双击启动。

- **在系统设置中允许**
  如果仍被阻止，进入 **系统设置 → 隐私与安全性**，向下滚动找到类似 *「已阻止 'Minions'，因为无法验证开发者」* 的提示，点击 **「仍要打开」** 或 **「允许」**。
...
详细使用说明、故障排除和常见问题，请参见 [桌面应用指南](https://minions.agentscope.io/docs/desktop)。
...
更喜欢待在终端里？运行 `minions` 即可打开一个全屏聊天 **TUI**，它驱动的是与控制台、IM 频道**完全相同**的智能体 — 同一份记忆、技能、MCP 工具与会话 — 无需离开键盘。
...
若使用**云端大模型 API**（如 DashScope / 通义千问、OpenAI、Anthropic、Google Gemini、DeepSeek、Kimi、OpenRouter 等），在开始对话前必须配置 API Key。未配置有效 Key 前，Minions 无法正常工作。详情请参考 [官方文档](https://minions.agentscope.io/docs/models)。

**配置方式：**

1. **控制台（推荐）** — 运行 `minions app` 后，打开 **http://127.0.0.1:8088/** → **设置** → **模型**。选择提供商、填写 **API Key**，并启用该提供商与模型。
2. **`minions init`** — 运行 `minions init` 时，会引导你配置 LLM 提供商与 API Key。按提示选择提供商并填写 Key 即可。
3. **环境变量** — 使用 DashScope 时，可在终端或工作目录下的 `.env` 文件中设置 `DASHSCOPE_API_KEY`。

其他工具所需密钥（如网页搜索的 `TAVILY_API_KEY`）可在控制台 **设置 → 环境变量** 中配置，详见 [配置](https://minions.agentscope.io/docs/config)。

> **仅用本地模型？** 若使用 [本地模型](#本地模型)（Minions Local / Ollama / LM Studio），则**无需**任何 API Key。

## 本地模型

Minions 可在本机完全本地运行大模型，无需 API Key 或云端服务。详情请见 [官方文档](https://minions.agentscope.io/docs/models)。

Minions 还提供 **Minions-Flash** 系列 — 面向 Agent 场景专项训练的 2B / 4B / 9B 模型，提供 Q4 与 Q8 量化版本。可在 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 与 [Hugging Face](https://huggingface.co/agentscope-ai/models) 获取。

| 后端                              | 适用场景                                 | 安装                                                                 |
| --------------------------------- | ---------------------------------------- | -------------------------------------------------------------------- |
| **Minions Local**（llama.cpp）    | 跨平台（macOS / Linux / Windows）        | 内置；在 Web 界面点击「Download」即可。支持 Minions-Flash，并提供硬件感知推荐。 |
...
- **File Guard** — 独立于 Tool Guard；阻止智能体访问敏感文件与目录（默认保护 `~/.minions.secret/`、`~/.ssh` 等）。
...
详见 [安全文档](https://minions.agentscope.io/docs/security)。
...
| 主题                                                                 | 说明                                             |
| -------------------------------------------------------------------- | ------------------------------------------------ |
| [项目介绍](https://minions.agentscope.io/docs/intro)                 | Minions 是什么、怎么用                           |
| [快速开始](https://minions.agentscope.io/docs/quickstart)            | 安装与运行（本地或魔搭创空间）                   |
| [控制台](https://minions.agentscope.io/docs/console)                 | Web 界面：对话与智能体配置                       |
| [终端界面（TUI）](https://minions.agentscope.io/docs/tui)            | 终端全屏对话，与控制台共用同一智能体             |
...
| [Cron / 定时任务](https://minions.agentscope.io/docs/cron)           | 定时任务与自动化                                 |
| [多智能体](https://minions.agentscope.io/docs/multi-agent)           | 创建多个智能体并启用协作                         |
| [安全](https://minions.agentscope.io/docs/security)                    | Sandbox、Tool Guard、File Guard、Skill Scanner、Access Policy |
| [备份与恢复](https://minions.agentscope.io/docs/backup)              | 数据备份与恢复                                   |
| [配置与工作目录](https://minions.agentscope.io/docs/config)          | 工作目录与配置文件                               |
| [REST API](https://minions.agentscope.io/docs/api-tutorial)          | 用于集成与自动化的 HTTP API                      |
| [ACP 集成](https://minions.agentscope.io/docs/acp-integration)       | Agent Communication Protocol 集成                |
| [CLI](https://minions.agentscope.io/docs/cli)                        | 初始化、定时任务、Skills、清理                   |
| [Agent 团队实践](https://minions.agentscope.io/docs/practice-agent-team) | 多智能体团队部署指南                         |
| [FAQ](https://minions.agentscope.io/docs/faq)                        | 常见问题与故障排查                               |

完整文档：[minions.agentscope.io/docs](https://minions.agentscope.io/docs)

---

## 常见问题

更多常见问题、故障排查技巧和已知问题，请访问 **[FAQ 页面](https://minions.agentscope.io/docs/faq)**。
...
Minions 在开放协作中持续演进，欢迎各种形式的参与！请参考上方 [路线图](#路线图)（尤其是标记为 **征集中** 的项）选择你感兴趣的方向，并阅读 [CONTRIBUTING](https://github.com/agentscope-ai/Minions/blob/main/CONTRIBUTING_zh.md) 了解如何开始。我们特别欢迎：

- **横向拓展** — 新频道、模型提供商、Skills、MCP。
- **已有功能扩展与完善** — 展示与交互优化、下载提示、Windows 路径兼容等。

欢迎在 [GitHub Discussions](https://github.com/agentscope-ai/Minions/discussions) 参与讨论、提出想法或认领任务。
...
- **然后**：运行 `minions init --defaults`，再运行 `minions app`。

> **版本更新提示：** 当执行 `git pull` 更新到大版本后，请重新构建前端、重新安装 Python 包（`pip install -e .`）、重启 `minions app`，并清除浏览器缓存（`Ctrl+Shift+R` 或 macOS 上 `Cmd+Shift+R`）。

---

## 为什么叫 Minions？

Minions 既是 Qwen Personal Agent Workstation（千问个人智能体工作台），也寓意 Qwen 的智识与 Paw 的温度。我们希望它不是冰冷的工具，而是一只随时准备帮忙的智慧、温暖「小爪子」，是你数字生活中最默契的伙伴。
...
<a href="https://github.com/agentscope-ai/Minions">
  <img src="https://img.alicdn.com/imgextra/i1/O1CN01V8HYv61By0HYcIDaq_!!6000000000013-1-tps-1698-954.gif" width="600" alt="Star Minions" />
</a>

在 GitHub 上 Star Minions，第一时间收到新版本发布通知。

---

## 遥测数据

Minions 在执行 `minions init` 时会收集**匿名**使用数据，帮助我们了解用户环境并优化产品。数据**每个版本收集一次** — 当你升级 Minions 后，会重新收集以便我们了解版本分布。
...
交互式运行 `minions init` 时，会询问你是否同意。使用 `--defaults` 模式则自动同意。提示每个版本仅出现一次，且不影响 Minions 的任何功能。

---

## 许可证

Minions 采用 [Apache License 2.0](LICENSE) 开源协议。

---

## 贡献者

感谢所有为 Minions 做出贡献的朋友们：

<a href="https://github.com/agentscope-ai/Minions/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/Minions" alt="贡献者" />
</a>
