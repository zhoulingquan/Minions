...
def _link_or_copy(source: Path, target: Path) -> None:
    """Link an overlay entry, copying only when link privileges are absent."""
    try:
        target.symlink_to(source, target_is_directory=source.is_dir())
        return
    except OSError as exc:
        permission_denied = exc.errno in {errno.EACCES, errno.EPERM} or getattr(
            exc,
            "winerror",
            None,
        ) == 1314
...
def _isolated_skills_workspace(
    ...
    with tempfile.TemporaryDirectory(prefix="minions_headless_") as tmp:
        tmp_path = Path(tmp)
        resolved = Path(skills_dir).resolve()
        _link_or_copy(resolved, tmp_path / "skills")
