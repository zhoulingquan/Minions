# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``minions doctor fix``.
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `minions doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `minions doctor fix` without "
        ...
        if repo is None:
            ...
            def _rebuild_console(
                ...
                if (
                    ...
                    bkp_root = r / ".minions-doctor-fix-backups"
                    sid = _utc_session_id()
                    bkp = bkp_root / sid
