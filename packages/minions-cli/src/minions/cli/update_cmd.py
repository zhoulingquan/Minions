...
from .process_utils import (
    ...
    _is_minions_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/minions/json"
...
class InstallInfo:
    """Information about the current Minions installation."""
...
class RunningServiceInfo:
    """Detected Minions service endpoint state."""
...
def _fetch_pypi_release_data() -> dict[str, Any]:
    """Fetch minions release metadata from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest Minions version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest Minions version: {exc}",
        ) from exc
    if not isinstance(data, dict):
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            "latest Minions version.",
        )
    return data
...
def _select_latest_version(
    ...
    *,
    ...
    if include_prerelease:
        ...
        if not version:
            raise click.ClickException(
                "Unable to determine the latest Minions version.",
            )
        return version
    ...
    if not candidates:
        raise click.ClickException(
            "Unable to determine the latest stable Minions version.",
        )
    return str(max(candidates))


def _fetch_latest_version(*, include_prerelease: bool = False) -> str:
    """Fetch the latest published Minions version from PyPI."""
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("minions")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running Minions HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local Minions service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_minions_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a Minions HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running Minions service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"Minions service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `minions shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING MINIONS SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `minions shutdown` will forcibly terminate the current "
        "Minions backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
...
def _run_shutdown_for_update(
    ...
    """Run `minions shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "minions"]
    ...
    click.echo("")
    click.echo("Running `minions shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `minions shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`minions shutdown` failed. Please stop the running Minions "
            "service manually before running `minions update`.",
        )
...
def _build_upgrade_command(
    ...
    *,
    ...
    package_spec = f"minions=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from minions.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[minions] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[minions] Updating Minions "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[minions] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[minions] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[minions] Update completed successfully.")
        click.echo(
            "[minions] Please restart any running Minions service "
            "to use the new version.",
        )
    else:
        click.echo(f"[minions] Update failed with exit code {return_code}.")
        click.echo(
            "[minions] Please fix the error above and run "
            "`minions update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool, prerelease: bool) -> None:
    """Upgrade Minions in the current Python environment."""
    ...
    if version_check is False:
        click.echo("Minions is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `minions update`, or rerun "
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `minions shutdown`. "
                "Please stop it manually before running `minions update`.",
            )

    if not yes and not click.confirm(
        f"Update Minions to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
