...
from ..core.protocols import ApprovalRequester

logger = logging.getLogger(__name__)


class GuardedFunctionTool:
    """Permission-checked tool wrapper using minions's tool-guard engine.

    Routes every tool invocation through
    ``minions.security.tool_guard.engine`` which evaluates guard rules and
...
def _guarded_tool_init(
    ...
    **kwargs: Any,
    ...
    # pylint: disable=protected-access
    self._qp_approval_requester = approval_requester
...
async def _guarded_tool_check_permissions(
    ...
    **_extra_kwargs: Any,
    ...
    """Drive minions's tool-guard engine + ApprovalService for one tool call.
    ...
    """
    del context  # minions's guard doesn't read PermissionContext yet
    ...
    decision = await _ask_user_approval(
        approval_requester=getattr(self, "_qp_approval_requester", None),
        agent_id=agent_id,
        tool_name=tool_name,
...
async def _ask_user_approval(
    *,
    ...
    """Request approval through an injected app-owned adapter."""
    from agentscope.permission import (
        PermissionBehavior,
        PermissionDecision,
    )
    ...
    if approval_requester is None:
        return PermissionDecision(
            ...
            message=_with_no_retry_instruction(
                "Tool approval requester is not configured; denying "
                f"'{tool_name}' by default.",
            ),
        )

    try:
        response = await approval_requester.request_approval(
            agent_id=agent_id,
            tool_name=tool_name,
            input_data=input_data,
            guard_result=guard_result,
