...
TURN_USAGE_META_KEY = "minions_turn_usage"
...
async def snapshot_context_usage_for_state(
    ...
    try:
        from .context_stats import estimate_context_tokens
        from .estimate_token_counter import EstimatedTokenCounter

        max_input_length = int(max_input_length or 0)
    ...
    *,
    ...
    if state:
        ...
        if agent_state is not None:
            ctx = await snapshot_context_usage_for_state(
                agent_state,
                max_input_length,
            )
