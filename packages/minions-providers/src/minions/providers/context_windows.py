...
and the display/usage path (``get_model_max_input_length`` below) go through
...
def get_model_max_input_length(agent_config: Any) -> int:
    """Return an agent's active model context window via its provider."""
    from .provider_manager import ProviderManager

    model_slot = agent_config.active_model
    if not model_slot or not model_slot.provider_id:
        try:
            model_slot = ProviderManager.get_instance().get_active_model()
        except Exception:
            pass
