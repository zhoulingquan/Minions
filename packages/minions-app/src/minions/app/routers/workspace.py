...
_SQLITE_TRANSIENT_SUFFIXES = ("-shm", "-wal", "-journal")
...
def _zip_directory(root: Path) -> io.BytesIO:
    ...
    All files **and** directories (including empty ones) are included except
    SQLite's live sidecar files, which are process-local transient state and
    may be memory-mapped or locked while the workspace is running.
    """
    buf = io.BytesIO()
