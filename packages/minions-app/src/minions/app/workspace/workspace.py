...
from .local_workspace import MinionsLocalWorkspace
...
class Workspace:
    ...
    def __init__(self, agent_id: str, workspace_dir: str):
        ...
        self._local_workspace = MinionsLocalWorkspace(
    ...
    @property
    def local_workspace(self) -> MinionsLocalWorkspace:
    ...
    async def stream_query(
        ...
        try:
            ...
            async with agent_runtime_scope(self.agent_id, source=source):
                from ...agents.runtime_builder import AgentBuilder
    ...
    async def stop(self, final: bool = True):
        ...
        active_runs = await self.task_tracker.list_active_tasks()
        for run_key in active_runs:
            await self.task_tracker.request_stop(run_key)
        if active_runs and not await self.task_tracker.wait_all_done(timeout=5.0):
            logger.warning(
                "Timed out waiting for %d active run(s) before stopping %s",
                len(active_runs),
                self.agent_id,
            )
