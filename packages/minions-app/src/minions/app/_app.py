...
from ..core.bootstrap import initialize_environment
...
from ..core.restore import cleanup_startup_restore_artifacts
...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        message = (
            "Minions startup failed because restore artifact cleanup did not "
    ...
    try:
        ...
        try:
            ...
            _api_action_command_specs.extend(collect_daemon_command_specs())
            # pylint: disable-next=protected-access
            workspace_registry._bootstrap_kwargs[
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            ...
            from ..channels.command_registry import CommandRegistry
...
if not _CONSOLE_STATIC_DIR:
    logger.warning("Console static resources are unavailable.")
_CONSOLE_INDEX = (
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get(
        "/{full_path:path}",
        name="minions_console_spa_catchall",
    )
