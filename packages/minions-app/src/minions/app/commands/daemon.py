# -*- coding: utf-8 -*-
"""Application-owned daemon command execution and handler mixin.
...
def _make_daemon_adapter(subcommand: str) -> CommandSpec:
    """Create a slash-command adapter for one daemon subcommand."""
    ...
    async def _handler(ctx: Any, args: str) -> Msg | None:
        ...
        try:
            ...
            agent_name = config.name if config and config.name else "Minions"
        except Exception:
            agent_name = "Minions"
        ...
        return await DaemonCommandHandlerMixin().handle_daemon_command(
            f"/{subcommand} {args}".strip(),
            daemon_context,
        )
...
def _make_daemon_compound_adapter() -> CommandSpec:
    """Create the ``/daemon <subcommand>`` compound adapter."""

    async def _handler(ctx: Any, args: str) -> Msg | None:
        from ...config.config import load_agent_config

        full_query = f"/daemon {args}".strip()
        if parse_daemon_query(full_query) is None:
            return Msg(
                name="assistant",
                role="assistant",
...
def collect_daemon_command_specs() -> list[CommandSpec]:
    """Return all app-owned daemon slash-command specs."""
    specs = [
    ...
    reload_spec = _make_daemon_adapter("reload-config")
    specs.append(
        CommandSpec(
            name=reload_spec.name,
            handler=reload_spec.handler,
            aliases=("reload_config",),
            category=reload_spec.category,
        ),
    )
