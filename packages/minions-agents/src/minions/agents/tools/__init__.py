# -*- coding: utf-8 -*-
"""Built-in tool functions for Minions agents.
...
are intentionally not re-exported — minions's react_agent does not
...
def discover_builtin_tool_funcs() -> list[Callable]:
    ...
    """
    from ...runtime.tool_registry import get_registered_tool_funcs
    ...
    return [
        func
        for func in get_registered_tool_funcs()
        if getattr(func, "__module__", "").startswith(
            "minions.agents.tools.",
        )
    ]
