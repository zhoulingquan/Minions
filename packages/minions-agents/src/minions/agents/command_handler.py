...
from ..providers.context_windows import get_model_max_input_length
...
class CommandHandler(ConversationCommandHandlerMixin):
    ...
    async def _process_compact(
        ...
        compress_stats: dict = {}
        try:
            # Agent-backed mode: ``MinionsAgent.compress_context`` already
    ...
    async def _get_current_system_prompt(self) -> str:
        ...
        if ctx is not None:
            try:
                from .runtime_builder import AgentBuilder
