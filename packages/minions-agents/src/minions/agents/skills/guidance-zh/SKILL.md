---
name: guidance
description: "回答用户关于 Minions 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
...
当用户询问 **Minions 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
# 逻辑推导：如果路径包含 .minions/bin/minions，则根目录在其上三层
# 例如：/path/to/Minions/.minions/bin/minions -> /path/to/Minions
...
如果还是找不到文档，搜索 minions 安装路径下的可用文档内容：
