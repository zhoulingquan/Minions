...
minions envelope schema and the ACP transport stack.  Eagerly importing
them here would slow down every consumer of ``minions.agents.acp`` —
including ``delegate_external_agent`` which only needs
``tool_adapter`` — so we expose ``MinionsACPAgent`` /
``run_minions_agent`` / ``ACPService`` / ``*_acp_service`` via
...
_LAZY_EXPORTS = {
    "MinionsACPAgent": ".server",
    "run_minions_agent": ".server",
    "ACPService": ".service",
...
# fine — verified by ``from minions.agents.acp import ACPService`` working.
__all__ = [
    "ACPErrors",
    "ACPConfigurationError",
    "ACPProtocolError",
