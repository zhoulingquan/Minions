# -*- coding: utf-8 -*-
"""Minions ACP Agent server.

Exposes Minions as an ACP-compliant agent that external clients
...
from .host import ACPHostServices
from .meta import (
...
ACP_ERROR_META_KEY = "minions.error"
ACP_AGENT_META_KEY = "minions.agent"
...
_GENERIC_PROMPT_ERROR = (
    "Minions failed to process the request. Check server logs for details."
)
...
class MinionsACPAgent(Agent):
    ...
    def __init__(
        ...
        host: ACPHostServices | None = None,
    ):
    ...
    def _require_host(self) -> ACPHostServices:
        """Return the configured host or fail before touching app services."""
        if self._host is None:
            raise RuntimeError("ACP host services are not configured")
        return self._host
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        workspace = await self._require_host().start_workspace(
            agent_id,
            workspace_dir,
        )
        ...
        logger.info(
            "Minions ACP Agent workspace started: agent_id=%s workspace=%s",
            agent_id,
            workspace_dir,
        )
        return workspace
    ...
    async def _shutdown_workspace(self) -> None:
        ...
        if self._workspace is not None:
            try:
                await self._require_host().stop_workspace(self._workspace)
            except Exception:
                logger.exception(
    ...
    async def _bridge_approval_requests(
        ...
        *,
        ...
        """Bridge Minions ApprovalService waits to ACP permission prompts."""
        host = self._require_host()
        seen: set[str] = set()
        while True:
            for pending in await host.get_pending_approvals(session_id):
    ...
    def _approval_options(self, pending: Any) -> list[PermissionOption]:
        """Build exact or generalized session-scoped approval options."""
        display = self._approval_display(pending)
        if (
    ...
    def _approval_tool_input(self, pending: Any) -> dict[str, Any] | None:
    ...
    def _approval_display(self, pending: Any) -> dict[str, Any]:
        try:
            return self._require_host().approval_display(pending)
        except Exception:
    ...
    async def _cancel_pending_approvals(self, session_id: str) -> None:
        try:
            await self._require_host().cancel_pending_approvals(session_id)
        except Exception:
            logger.debug(
    ...
    def _session_meta(self) -> dict[str, Any] | None:
        """Return session ``_meta`` with the resolved Minions agent id."""
        try:
            agent_id = self._resolve_agent_id()
        except Exception:
...
async def run_minions_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
    local_diagnostics: bool = False,
    host: ACPHostServices | None = None,
) -> None:
    """Entry point: run Minions as an ACP agent over stdio."""
