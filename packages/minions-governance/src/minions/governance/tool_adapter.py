...
def _is_execution_level_off() -> bool:
    ...
    try:
        ...
        policy_path = Path(WORKING_DIR) / ".minions" / "policy.yaml"
...
def _policy_tool_init(
    ...
    **kwargs: Any,
    ...
    self._qp_model_factory = model_factory
    ...
    **_extra_kwargs: Any,
    ...
    if decision.action is GovernanceAction.ALLOW:
        ...
        return await _ask_user_approval(
            governor=governor,
            tc_spec=tc_spec,
            approval_requester=getattr(
                self,
                "_qp_approval_requester",
                None,
            ),
            model_factory=getattr(self, "_qp_model_factory", None),
...
async def _ask_user_approval(
    ...
    *,
    ...
    model_factory: Callable[..., tuple[Any, Any]] | None,
    ...
    if approval_requester is None:
        return PermissionDecision(
            behavior=PermissionBehavior.DENY,
            message=(
                "Governance approval requester is not configured; "
                f"denying '{tool_name}' by default."
                + _NO_RETRY_INSTRUCTION
            ),
        )
