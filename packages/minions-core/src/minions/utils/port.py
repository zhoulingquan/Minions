...
MINIONS_DESKTOP_PORT environment variable.
"""
...
from ..constant import MINIONS_DESKTOP_PORT
...
def get_stable_port(
    ...
    If MINIONS_DESKTOP_PORT environment variable is set, it takes priority
    and the port is fixed (not persisted).
    """
    # Check for user-configured port via environment variable
    if MINIONS_DESKTOP_PORT:
        try:
            port = int(MINIONS_DESKTOP_PORT)
        except (TypeError, ValueError):
            logger.warning(
                "MINIONS_DESKTOP_PORT=%r is not a valid "
                "integer, falling back to auto-assign",
                MINIONS_DESKTOP_PORT,
            )
            port = None
        if port is not None:
            if not 1024 <= port <= 65535:
                logger.warning(
                    "MINIONS_DESKTOP_PORT=%d out of range "
                ...
                if sock:
                    logger.info(
                        "Using MINIONS_DESKTOP_PORT: %d",
                        port,
                    )
                    return port, sock
                logger.warning(
                    "MINIONS_DESKTOP_PORT=%d is "
