...
import type { HarvestInstance, MsgSummary, PushMessage } from "../types";
...
export const useMsgData = () => {
  ...
  const loadPushMessages = useCallback(async () => {
    try {
      const res = await api.getMsgEvents({ limit: 200 });
      ...
      console.error("Failed to fetch push msg data", error);
    }
  }, []);
  ...
  const markMessageAsRead = useCallback((messageId: string) => {
    void api.markMsgRead({ event_ids: [messageId] });
  ...
  const markAllMessagesAsRead = useCallback(async (): Promise<number> => {
    ...
    await api.markMsgRead({ all: true });
    setPushMessages((prev) =>
      prev.map((message) =>
  ...
  const deleteMessages = useCallback(async (messageIds: string[]) => {
    ...
    await Promise.allSettled(ids.map((id) => api.deleteMsgEvent(id)));
    let deleted = 0;
    let unreadDeleted = 0;
    setPushMessages((prev) => {
