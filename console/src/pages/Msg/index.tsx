...
const MSG_TAB_STORAGE_KEY = "minions.msg.activeTab";
...
const resolveInitialTab = (): TabKey => {
  ...
  const stored = window.localStorage.getItem(MSG_TAB_STORAGE_KEY);
...
export default function MsgPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(MSG_TAB_STORAGE_KEY, activeTab);
    }
  }, [activeTab]);
