# `minions_pet_desktop` (package internals)

Importable Python package embedded inside the **Minions Pet plugin**.
It ships the desktop runtime: a transparent always-on-top Qt pet
window + a local FastAPI bridge on `127.0.0.1:8765` that Minions (via
...
the package is importable as `minions_pet_desktop` from Minions's
...
| `__main__.py` | `python -m minions_pet_desktop` → delegates to `cli.main`. |
| `cli.py` | CLI subcommands (`python -m minions_pet_desktop ...`): `init`, `start`, `stop`, `status`, `install-pet`, `install-default-pet`, `token`, `switch`, `send`. |
...
| `server.py` | FastAPI app factory (`build_app`). Defines `/health`, `/event`, `/state`, `/bubble`, `/pet`. Optional bearer-style token guard via `MINIONS_PET_REQUIRE_TOKEN=1` + `X-Minions-Pet-Token`. |
...
| `assets/default-pet/snowpaw/` | The bundled default pet (`pet.json` + `spritesheet.webp`). Loaded via `importlib.resources.files("minions_pet_desktop")`, which resolves relative to this folder on disk (no setuptools install required). |

## Runtime layout

`runtime.home_dir()` defaults to `~/.minions-pet/` (override with
`MINIONS_PET_HOME`) and contains:
...
~/.minions-pet/runtime/
  ...
  pet-desktop.log    # stdout/stderr of detached `minions-pet start`
```
...
send `X-Minions-Pet-Token: <token from runtime/update-token>` on
mutating endpoints (the bundled Minions plugin reads the token file
automatically). To disable the check (e.g. for local development), set
`MINIONS_PET_REQUIRE_TOKEN=0`.
...
These two are equivalent and bypass the `minions-pet start` daemon
wrapper (no PID file, no log redirection — useful while iterating):

```bash
python -m minions_pet_desktop.app --port 8765 --scale 0.58
python -m minions_pet_desktop                 # → cli.main, takes subcommands
```
