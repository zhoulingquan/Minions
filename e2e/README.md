...
> your real `~/.minions` you will **corrupt your actual Minions data**.
>
> The framework enforces this rule: if `MINIONS_WORKING_DIR` is unset
...
# This starts Minions on port 7077 with:
...
```bash
export MINIONS_WORKING_DIR=/tmp/my-e2e-workdir
mkdir -p "$MINIONS_WORKING_DIR"
MINIONS_WORKING_DIR="$MINIONS_WORKING_DIR" python -m minions app --port 7077 &
```

### 1. Install dependencies

```bash
cd /Users/ming/.minions/workspaces/Hv3HJ9
pip install -r tests/requirements.txt
playwright install chromium
```

### 2. Ensure the Minions service is running

```bash
minions start
# or
cd /Users/ming/Desktop/minions && python -m minions
```
...
```bash
MINIONS_HEADLESS=false pytest tests/tests/test_chat_p0.py -v
```
...
|------|---------|-------------|
| `MINIONS_WORKING_DIR` | **(required)** | Backend working directory for seed data. Must be outside `$HOME`. |
| `MINIONS_BASE_URL` | `http://localhost:7077` | Minions service URL |
| `MINIONS_HEADLESS` | `true` | Headless mode (`true`/`false`) |
| `MINIONS_TIMEOUT` | `30000` | Timeout (milliseconds) |
| `MINIONS_USER_ID` | `default` | User ID |
| `MINIONS_CHANNEL` | `console` | Channel name |
...
MINIONS_BASE_URL=http://127.0.0.1:9000 pytest tests/tests/test_chat_p0.py -v

# Headed mode + slow motion
MINIONS_HEADLESS=false PLAYWRIGHT_SLOW_MO=500 pytest tests/tests/test_chat_p0.py -v

# Increase timeout
MINIONS_TIMEOUT=60000 pytest tests/tests/test_chat_p0.py -v
```
...
### 1. Test failure: cannot connect to the Minions service
...
MINIONS_HEADLESS=false pytest tests/tests/test_chat_p0.py::TestNewChatAndBasicQA -v

# Increase timeout
MINIONS_TIMEOUT=60000 pytest tests/tests/test_chat_p0.py -v

# Use slow motion to inspect page loading
PLAYWRIGHT_SLOW_MO=1000 MINIONS_HEADLESS=false pytest tests/tests/test_chat_p0.py -v
```
