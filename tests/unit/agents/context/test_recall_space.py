...
"""Unit tests for :class:`RecallSpace` — the model's SQLite recall surface.
...
@pytest.fixture
def ms(history_db: Path) -> RecallSpace:
    space = RecallSpace(
...
def test_authorizer_blocks_escape_attempts(ms: RecallSpace, sql: str):
...
def test_scratch_is_read_write(ms: RecallSpace):
...
def test_hist_is_readable(ms: RecallSpace):
    rows = ms.sql_query(
...
def test_search_default_is_this_agent_cross_session(ms: RecallSpace):
...
def test_search_rows_carry_session_id(ms: RecallSpace):
...
def test_search_all_agents_spans_the_workspace(ms: RecallSpace):
    contents = {r["content"] for r in ms.search("tanks", all_agents=True)}
    assert "tanks of another agent" in contents
    assert len(contents) == 3


def test_search_pins_to_an_explicit_session(ms: RecallSpace):
...
def test_search_pins_to_an_explicit_agent(ms: RecallSpace):
...
def test_explicit_target_takes_precedence(ms: RecallSpace):
...
def test_like_fallback_respects_scope(ms: RecallSpace):
...
def test_like_fallback_emits_degradation_notice(ms: RecallSpace):
...
def test_no_notice_when_fts_available(ms: RecallSpace):
...
def test_expand_returns_full_turns_in_span(ms: RecallSpace):
    rows = ms.expand(1, 99)
...
def test_sessions_lists_this_agents_conversations(ms: RecallSpace):
...
def test_sessions_all_agents_spans_the_workspace(ms: RecallSpace):
    ids = {r["session_id"] for r in ms.sessions(all_agents=True)}
    assert ids == {"s1", "s2", "s3"}


def test_session_is_agent_scoped_by_default(ms: RecallSpace):
...
def test_agents_is_workspace_wide(ms: RecallSpace):
