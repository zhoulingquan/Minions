...
"""
PyInstaller spec file for Minions Desktop (Tauri sidecar).
...
bundle also includes a minions CLI executable for the Windows installer PATH
option.
"""
...
# from minions/console, so Tauri can navigate to the backend-hosted same-origin
...
_data_dirs = [
    ("agents/skills", "minions/agents/skills"),
    ("agents/md_files", "minions/agents/md_files"),
    ("tokenizer", "minions/tokenizer"),
    ("security/tool_guard/rules", "minions/security/tool_guard/rules"),
    ("security/skill_scanner/rules", "minions/security/skill_scanner/rules"),
    ("security/skill_scanner/data", "minions/security/skill_scanner/data"),
    ("app/channels/yuanbao/proto", "minions/app/channels/yuanbao/proto"),
]
datas = [
    (str(SRC / src), dst) for src, dst in _data_dirs if (SRC / src).is_dir()
]
datas += collect_tree(CONSOLE_DIST, "minions/console")
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        *collect_submodules("minions.cli"),
        # All channel adapters (imported on-demand at runtime)
        *collect_submodules("minions.app.channels"),
        # ACP runner support is lazily imported by delegate_external_agent.
        *collect_submodules("minions.agents.acp"),
        # ASGI app entry points
        "minions.app._app",
        "minions.app.multi_agent_manager",
        "minions.app.chats",
        "minions.app.task_tracker",
        "minions.runtime.commands",
        # Backup modules are exposed through minions.backup.__getattr__, which
        # PyInstaller cannot discover from static imports.
        *collect_submodules("minions.backup"),
