...
$Archive = Join-Path $Dist "minions-env.zip"
...
$LauncherBat = Join-Path $EnvRoot "Minions Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var MINIONS_LOG_LEVEL or default to "info"
if not defined MINIONS_LOG_LEVEL set "MINIONS_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\minions_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.minions\config.json" (
  "%~dp0python.exe" -u -m minions init --defaults --accept-security
)
"%~dp0python.exe" -u -m minions desktop --log-level %MINIONS_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "Minions Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with MINIONS_LOG_LEVEL)
...
echo [Launch] Starting Minions Desktop with log-level=%MINIONS_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m minions desktop --log-level %MINIONS_LOG_LEVEL%
echo.
echo [Exit] Minions Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "Minions Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\Minions Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create minions.cmd wrapper in env root so "minions" resolves to this
# instead of Scripts\minions.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$MinionsCmd = Join-Path $EnvRoot "minions.cmd"
@"
@"%~dp0python.exe" -u -m minions %*
"@ | Set-Content -Path $MinionsCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('minions'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] MINIONS_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "Minions-Setup-$Version.exe"
