# -*- coding: utf-8 -*-
"""minions uninstall — remove the Minions environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove Minions PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# Minions" comment line and the export PATH line
    cleaned = re.sub(
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove Minions environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL Minions data in {wd}")
    else:
        click.echo(
            "This will remove the Minions Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("Minions uninstalled. Please restart your terminal.")
