...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  minions agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  minions agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      minions agents chat --from-agent bot_a --to-agent bot_b --text "..."
      minions agents create --name "Research Bot" --agent-id research_bot
      minions agents delete research_bot
      minions agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      minions agents list
      minions agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `minions agents list`.

    \b
    Examples:
      minions agents delete research
      minions agents delete research --remove-workspace
      minions agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'minions agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'minions chats list' to find existing sessions (optional)
