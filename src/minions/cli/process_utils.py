...
def _matches_minions_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a Minions CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m minions {subcommand}",
            f" minions {subcommand}",
            f"__main__.py {subcommand}",
            f'minions.exe" {subcommand}',
            f"minions.exe {subcommand}",
        )
    )


def _is_minions_service_command(command: str) -> bool:
    """Return whether the command line looks like a local Minions app."""
    return _matches_minions_cli_command(command, "app")


def _is_minions_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a Minions CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "minions" or _matches_minions_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local Minions service."""
    if not host:
        return []
