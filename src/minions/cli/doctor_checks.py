# -*- coding: utf-8 -*-
"""Read-only diagnostics for `minions doctor` (no config or disk mutations)."""
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `minions app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``minions app``), when ``GET /api/doctor/runtime`` returned
    ...
    lines = [
        ...
        f"minions version: {__version__}",
        f"platform: {platform.system()} {platform.machine()}",
        f"doctor_python_environment: {doctor_env}",
    ]
    if server_python_environment is not None:
        lines.append(
            f"minions_python_environment: {server_python_environment}",
        )
    else:
        lines.append(
            "minions_python_environment: "
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``minions doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``minions app`` may run an automatic migration;
    ...
    return (
        ...
        "`minions app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `minions app` once (or see docs / `minions init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("MINIONS_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "MINIONS_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- Minions checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~minions.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `minions doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def minions_local_llm_deep_notes() -> list[str]:
...
async def check_enabled_agents_model_connections(
    ...
    *,
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if deep and pid in _MINIONS_LOCAL_PROVIDER_IDS:
            # Only add once (same underlying llama.cpp runtime).
            if not any("llama.cpp" in n for n in notes):
                notes.extend(minions_local_llm_deep_notes())
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    notes: list[str] = []
    env_dir = EnvVarLoader.get_str("MINIONS_CONSOLE_STATIC_DIR", "").strip()
    if env_dir:
        notes.append(
