# -*- coding: utf-8 -*-
"""
Skill security scanner for Minions.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``MINIONS_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = EnvVarLoader.get_str("MINIONS_SKILL_SCAN_MODE") or None
    if env is not None:
        val = env.lower().strip()
...
_WORKING_DIR_CURRENT_NAME = ".minions"
