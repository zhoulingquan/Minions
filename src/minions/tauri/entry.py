...
from minions.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from minions.tauri.sidecar_logging import install_sidecar_logging
...
def _bundled_python() -> str:
    ...
    python = (os.environ.get("MINIONS_DESKTOP_PY_RUNTIME") or "").strip()
...
def _child_env_with_plugin_site(env: "dict | None") -> "dict | None":
    ...
    """
    site_dir = (os.environ.get("MINIONS_PLUGIN_SITE") or "").strip()
    if not site_dir:
        return env
...
def _reexec_as_bundled_python(args: Sequence[str]) -> None:
    ...
    plugin-installed deps (``MINIONS_PLUGIN_SITE``) importable.
    """
    python = _bundled_python()
    if not python:
        print(
            "minions-backend is the desktop backend, not a Python "
...
def _install_subprocess_guard() -> None:
    ...
    * On Windows, suppress console windows for child processes that don't pass
    ...
    if getattr(subprocess.Popen, "_minions_guarded", False):
        return
    ...
    setattr(subprocess.Popen, "_minions_guarded", True)


def _ensure_minions_app_not_loaded() -> None:
    if "minions.app._app" in sys.modules:
        raise RuntimeError(
            "minions app imported before desktop CORS origins were set",
        )


def _sync_loaded_minions_constant_cors_origins() -> None:
    constant_module = sys.modules.get("minions.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from minions.constant.CORS_ORIGINS at import time.
    _ensure_minions_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_minions_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from minions.config.utils import write_last_api
    from minions.constant import LOG_LEVEL_ENV, WORKING_DIR
    from minions.utils.logging import (
        SuppressPathAccessLogFilter,
        setup_logger,
    )
    from minions.utils.port import get_stable_port, write_port_file
    ...
    from minions.tauri.backend_guard import reconcile_singleton_backend

    reconcile_singleton_backend(WORKING_DIR)
    if normalized_log_level in ("debug", "trace"):
        from minions.cli.main import log_init_timings
...
def main() -> None:
    ...
    if not config_path.exists():
        from minions.cli.init_cmd import init_cmd
