...
def get_available_channels() -> Tuple[str, ...]:
    ...
    minions.channels), filtered by MINIONS_ENABLED_CHANNELS or
    MINIONS_DISABLED_CHANNELS when set.

    * MINIONS_ENABLED_CHANNELS — whitelist (only these channels are active).
    * MINIONS_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, MINIONS_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("MINIONS_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env MINIONS_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_minions_running() -> bool:
    """Check if Minions is currently running by checking API availability.

    Returns:
        True if Minions is running, False otherwise
    """
    try:
