# -*- coding: utf-8 -*-
"""Embedded ReMe application configuration for Minions memory.

ReMe's standalone CLI normally loads YAML such as
``reme/config/default.yaml`` or ``reme/config/minions.yaml``.  Minions embeds
...
from minions.config.config import AgentProfileConfig, EmbeddingModelConfig
...
def build_reme_app_config(
    *,
    ...
    """Build ReMe ``Application`` kwargs for embedded Minions usage."""
...
def _base_config(enable_search_raw_log: bool = False) -> dict[str, Any]:
    """Return the ReMe config shape used by Minions."""
...
def _apply_embedding_config(
    ...
    """Map Minions embedding config into ReMe component config."""
