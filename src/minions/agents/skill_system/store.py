...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "minions", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.minions.emoji."""
    if not isinstance(metadata, dict):
        return ""
    minions = metadata.get("minions")
    if isinstance(minions, dict):
        return str(minions.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="minions_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"minions_skill_stage_{skill_name}_"),
    )
