...
_TRACE_DIR = WORKING_DIR / "msg_traces"
_LOCK = asyncio.Lock()


def _migrate_legacy_inbox_traces() -> None:
    """One-shot migration: rename legacy ``inbox_traces/`` -> ``msg_traces/``."""
    legacy = WORKING_DIR / "inbox_traces"
    if legacy.exists() and not _TRACE_DIR.exists():
        try:
            legacy.rename(_TRACE_DIR)
        except OSError:
            pass
